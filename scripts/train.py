...
def train(model, optimizer, train_dataset, test_dataset, num_epochs, batch_size, model_dir):
	...
	for current_epoch in tqdm(range(num_epochs)):
		...
		for batch_idx, (data, targets) in enumerate(train_dataset):
			...
			y_true 		+= list(targets.int().cpu())
			y_pred 		+= list(pred_idx.data.int().cpu())
			total_loss 	+= loss
...
def create_plot(epoch_list,acc_list,directory):
	...
	plt.title("Training Accuracy over " + str(epoch) + " Epochs + " + str(features) + " Hidden size")
	plt.ylabel("Accuracy")
	plt.xlabel("Epoch")
	#plt.show()
...
if __name__ == '__main__':
	...
	parser.add_argument('--hidden_size',	type=int, default=features,
